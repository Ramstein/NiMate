    ...
    def run(self, objects, set_location_func, set_rotation_func):
        ...
        if sync:            
            for key, value in apply_location_dict.items():
                set_location_func(objects, key, value, self.original_locations)# Vector = value

####################Vector Axises
                '''w The w value (if any).
                    x The x value.  value.x
                    y The y value.  value.y
                    z The z value (if any).  value.z'''
            for key, value in apply_rotation_dict.items():
                set_rotation_func(objects, key, value, self.original_rotations)# Vector = value
...
####################Vector Axises
            ...
            for key, value in self.location_dict.items():
                set_location_func(objects, key, value, self.location_dict) # Vector = value
...
####################Vector Axises
            ...
            for key, value in self.rotation_dict.items():
                set_rotation_func(objects, key, value, self.rotation_dict)  #  Vector = value
####################Vector Axises
                '''w The w value (if any).
                    x The x value.  value.x
                    y The y value.  value.y
    ...
    def __init__(self, UDP_PORT, QUIT_PORT):
        ...
        self.rotation_dict = {}
###########################  test   #######################
        path = r'C:\Users\Ramstein\Desktop\VidoeTo Mocap_softwares\ni mate'
        os.makedirs(path, exist_ok=True)
        with open(os.path.join(path,'testProduced.txt'), 'w') as f:
            for i in range(10000, 10200):
                i = str(i)+'  Hello this is the {} time reminding you about it'.format(str(i))
                f.writelines(i)

        # Read back in
        with open(os.path.join(path,'testProduced.txt'), 'r') as file:
            data = file.read()
            # print("Imported integers:", data)
        ######################
